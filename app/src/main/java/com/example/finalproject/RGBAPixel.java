...
public class RGBAPixel {
    ...
    public int setRed(final int setValue) {
        if (setValue < 0) {
            return this.setComponent(PixelChannels.RED, 0);
        } else if (setValue > MAX_CHANNEL_VALUE) {
            return this.setComponent(PixelChannels.RED, MAX_CHANNEL_VALUE);
        }
    ...
    public int setGreen(final int setValue) {
        if (setValue < 0) {
            return this.setComponent(PixelChannels.GREEN, 0);
        } else if (setValue > MAX_CHANNEL_VALUE) {
            return this.setComponent(PixelChannels.GREEN, MAX_CHANNEL_VALUE);
        }
    ...
    public int setBlue(final int setValue) {
        if (setValue < 0) {
            return this.setComponent(PixelChannels.BLUE, 0);
        } else if (setValue > MAX_CHANNEL_VALUE) {
            return this.setComponent(PixelChannels.BLUE, MAX_CHANNEL_VALUE);
        }
    ...
    public int setAlpha(final int setValue) {
        if (setValue < 0) {
            return this.setComponent(PixelChannels.ALPHA, 0);
        } else if (setValue > MAX_CHANNEL_VALUE) {
            return this.setComponent(PixelChannels.ALPHA, MAX_CHANNEL_VALUE);
        }
    ...
    public static RGBAPixel blur(RGBAPixel[][] input, int amountX, int amountY) {
        ...
        for (int i = 0; i < amount; i++) {
            for (int j = 0; j < amount; j++) {
                ...
                result[i][j].setGreen(averageOne);
    ...
    public static String diffArrays(final RGBAPixel[][] firstArray,
        ...
        if (firstArray.length != secondArray.length) {
            ...
            for (int p = 0; p < firstArray.length; p++) {
                if (firstArray[p].length == secondArray[p].length) {
                    for (int c = 0; c < firstArray[p].length; c++) {
                        if (firstArray[p][c] != secondArray[p][c]) {
                            return "firstArray[" + p + "][" + c + "] != secondArray[" + p + "][" + c + "]";
                        }
                    }
                }
